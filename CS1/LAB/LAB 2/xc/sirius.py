...
# Function to run when mouse iks pressed:
...
# Function to un when mouse is released:
def mouse_released(mx, my):
    global earth, sun, solar_system, x, y, press_detector
    if press_detector:
        end_time = time.time()
        x, y = mx - WINDOW_WIDTH / 2, my - WINDOW_HEIGHT / 2
        x /= PIXELS_PER_METER
        y /= PIXELS_PER_METER
        dx = sun.x - x
        dy = sun.y - y
...
# body = Body(mass, x, y, vx, vy, pixel_radius, r, g, b)

# Two identical suns:
sun_mass = 1.98892e30
sun_separation = 3.22e9
sun_velocities = math.sqrt(G * sun_mass / (13 * sun_separation))
sun1 = Body(1.98892e30, 0, sun_separation / 2, 0 - sun_velocities, 0, 15, 0.93, 0.62, 0.05)
sun2 = Body(1.98892e30, 0, 0 - sun_separation / 2, sun_velocities, 0, 15, 0.93, 0.62, 0.05)

# Nine planets
mercury = Body(3.1e23, 8.7 * 5.79e7, 0, 0, 0.788 * 47.4e4, 2.87, 0, 1, 0)
venus = Body(4.87e24, 7 * 1.082e8, 0, 0, 0.81 * 35.0e4, 3.104, 1, 1, 1)
earth = Body(5.97e24, 7 * 1.496e8, 0, 0, 1 * 29.8e4, 6.756, 0, 0, 1)
...
neptune = Body(1.02e26, 4.495e9, 0, 0, 3.5 * 5.4e4, 4.9528, 0.4, 0, 0.4)
pluto = Body(1.4e22, 0.9 * 5.906e9, 0, 0, 3.50 * 4.7e4, 2.37, 0.8, 0.8, 0)
sun = sun1
list_of_bodies = [sun1, sun2, mercury, venus, earth]

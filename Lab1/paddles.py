...
PADDLE_WIDTH, PADDLE_HEIGHT,  PADDLE_VELOCITY = 20, 80, 20
...
# Ball stuff:
BALL_RADIUS = 20
BALL_INITIAL_VELOCITY_X, BALL_INITIAL_VELOCITY_Y = -5, 3
...
def draw():
    ...
    # global x1, y1, x2, y2
    global key_a, key_z, key_k, key_m, key_space, key_q
    global PADDLE_VELOCITY, PADDLE_WIDTH, PADDLE_HEIGHT, LEFT_BAR_X, left_bar_y, RIGHT_BAR_X, right_bar_y
    ...
    def paddle_movement():
        ...
        global PADDLE_VELOCITY, PADDLE_HEIGHT, WINDOW_HEIGHT

        # Left bar
        if not (key_a and key_z):

            # Up Movement:
            if key_a and left_bar_y - PADDLE_VELOCITY >= 0:
                left_bar_y -= PADDLE_VELOCITY  # Up Movement
            elif key_a:
                left_bar_y = 0
            # Down Movement:
            if key_z and left_bar_y + (PADDLE_HEIGHT + PADDLE_VELOCITY) <= WINDOW_HEIGHT:
                left_bar_y += PADDLE_VELOCITY  # Down Movement
            elif key_z:
        ...
        if not (key_k and key_m):
            ...
            # Up Movement:
            if key_k and right_bar_y - PADDLE_VELOCITY >= 0:
                right_bar_y -= PADDLE_VELOCITY  # Up Movement
            elif key_k:
                right_bar_y = 0

            # Down Movement:
            if key_m and right_bar_y + (PADDLE_HEIGHT + PADDLE_VELOCITY) <= WINDOW_HEIGHT:
                right_bar_y += PADDLE_VELOCITY  # Down Movement
            elif key_m:
